...
class Dataset():
    ...
    def read_df(self): 
        if os.path.exists(file):
            ...
            return pd.read_csv(self.df_csv)
...
def get_inn_list():
     inn_filename = get_inn_list_filename()
     return [r[0] for r in csv_stream(inn_filename)]
...
class INN_Subset(Dataset):
    ...
    def __init__(self, year, inn_list):        
        super().__init__(year)        
        self.output_csv=config.make_path_inn_subset_csv(year)
        self.inn_list=get_inn_list()
            
    # overload using *inn_list* filter
    def to_csv(self):
        gen = emit_rows(year, self.inn_list)            
        print ("Saving %s dataset with INN filter..." % year)        
        columns=get_colnames()        
        to_csv(self.output_csv, gen, columns) 
